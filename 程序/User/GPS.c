...
int GPS_RMC_Parse(char *line,GPS_INFO *GPS)
{
    ...
    if (ch == 'C') 
    {
        ...
        if (status == 'A')  //如果数据有效，则分析
        {
            ...
            latitude_Degree_tmp   = (int)GPS->latitude;       //获取度
					  lati_cent_tmp         = (GPS->latitude - latitude_Degree_tmp * 100);//获取分
					  GPS->latitude_Degree  = lati_cent_tmp  + latitude_Degree_tmp;//转化为ddd.dddddd格式
					
            longitude_Degreetmp   = (int)GPS->longitude ;	//获取度
            long_cent_tmp         = (GPS->longitude - longitude_Degreetmp * 100);//获取分
					  GPS->longitude_Degree = long_cent_tmp + longitude_Degreetmp;
